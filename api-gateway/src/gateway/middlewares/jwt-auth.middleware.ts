...
export class JwtAuthMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction) {
		...
		try {
			...
			this.logger.log(`JWT verified for user: ${(decoded as any).userId || "unknown"}`, "JwtAuthMiddleware");

			next();
		} catch (error) {
			this.logger.error(`JWT verification failed: ${error.message}`, error.stack, "JwtAuthMiddleware");
