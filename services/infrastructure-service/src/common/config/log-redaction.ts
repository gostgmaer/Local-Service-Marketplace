...
const SENSITIVE_PATTERNS: Array<{ regex: RegExp; replacement: string }> = [
  ...
  { regex: /"(password|passwd|secret|token|accessToken|refreshToken|access_token|refresh_token|api_key|apiKey|authorization|jwt|credit_card|card_number|cvv|ssn)"\s*:\s*"[^"]*"/gi, replacement: '"$1":"[REDACTED]"' },
  // Query string parameters: password=value&
  { regex: /(password|token|secret|api_key|apiKey|access_token|refresh_token)=([^&\s]+)/gi, replacement: '$1=[REDACTED]' },
  ...
  { regex: /\beyJ[A-Za-z0-9\-_]+\.eyJ[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+/g, replacement: '[REDACTED_JWT]' },
];
...
function redactObject(obj: any): any {
  ...
  const sensitiveKeys = /^(password|passwd|secret|token|accessToken|refreshToken|access_token|refresh_token|api_key|apiKey|authorization|jwt|credit_card|card_number|cvv|ssn)$/i;
