...
describe("list-query-validation.util", () => {
  ...
  it("derives page from offset when page is omitted", () => {
    expect(resolvePagination({ limit: 10, offset: 30 })).toEqual({
      page: 4,
      limit: 10,
      offset: 30,
    });
  ...
  it("rejects conflicting page and offset values", () => {
    expect(() => resolvePagination({ page: 2, limit: 20, offset: 5 })).toThrow(
      BadRequestException,
    );
  });
  ...
  it("rejects invalid min max ranges", () => {
    expect(() => validateMinMaxRange(5, 1, "attempts")).toThrow(
      BadRequestException,
    );
  });
});
