...
describe("FeatureFlagService", () => {
  ...
  describe("createFeatureFlag", () => {
    ...
    it("throws and logs on failure", async () => {
      ...
      featureFlagRepository.createFeatureFlag.mockRejectedValue(
        new Error("duplicate key"),
      );

      await expect(
        service.createFeatureFlag({ key: "dup", enabled: true } as any),
  ...
  describe("getFeatureFlagByKey", () => {
    ...
    it("falls back to database on cache miss", async () => {
      ...
      expect(featureFlagRepository.getFeatureFlagByKey).toHaveBeenCalledWith(
        "db_flag",
      );
  ...
  describe("getAllFeatureFlags", () => {
    it("returns paginated feature flags", async () => {
      ...
      featureFlagRepository.findFeatureFlags.mockResolvedValue([
        { id: "ff-1" },
      ]);
    ...
    it("rejects invalid rollout percentage range", async () => {
      ...
      await expect(
        service.getAllFeatureFlags({
          minRolloutPercentage: 80,
          maxRolloutPercentage: 20,
        } as any),
      ).rejects.toThrow();
    });
  });
