...
export class BackgroundJobService {
  ...
  async createJob(
    ...
    try {
      ...
      await this.jobQueue.add(createJobDto.jobType, {
        jobId: job.id,
  ...
  async getAllJobs(
    ...
    try {
      ...
      validateDateRange(
        queryDto.scheduledFrom,
        queryDto.scheduledTo,
        "scheduledFor",
      );
      validateMinMaxRange(
        queryDto.minAttempts,
        queryDto.maxAttempts,
        "attempts",
      );
  ...
  async updateJobStatus(
    ...
    try {
      const job = await this.backgroundJobRepository.updateJobStatus(
        id,
        status,
      );
