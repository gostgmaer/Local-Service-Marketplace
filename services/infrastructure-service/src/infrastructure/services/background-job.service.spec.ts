...
describe("BackgroundJobService", () => {
  const createService = () => {
    ...
    const jobQueue = {
      ...
      getJobCounts: jest
        .fn()
        .mockResolvedValue({ waiting: 0, active: 0, completed: 1, failed: 0 }),
    } as any;
  ...
  describe("createJob", () => {
    ...
    it("throws and logs on failure", async () => {
      ...
      backgroundJobRepository.createJob.mockRejectedValue(
        new Error("DB error"),
      );

      await expect(
        service.createJob({ jobType: "fail", payload: {} } as any),
  ...
  describe("getJobById", () => {
    it("retrieves a job by ID", async () => {
      ...
      backgroundJobRepository.getJobById.mockResolvedValue({
        id: "job-1",
        status: "completed",
      });
  ...
  describe("getAllJobs", () => {
    ...
    it("rejects invalid date range", async () => {
      ...
      await expect(
        service.getAllJobs({
          createdFrom: new Date("2026-03-20"),
          createdTo: new Date("2026-03-10"),
        } as any),
      ).rejects.toThrow();
    });
    ...
    it("rejects invalid attempts range", async () => {
      ...
      await expect(
        service.getAllJobs({ minAttempts: 10, maxAttempts: 2 } as any),
      ).rejects.toThrow();
    });
  });
});
