...
import { resolvePagination, validateMinMaxRange } from "../../common/pagination/list-query-validation.util";
...
export class FeatureFlagService {
	...
	async getFeatureFlagByKey(key: string): Promise<FeatureFlag | null> {
		try {
			...
			if (cached) {
				this.logger.log(`Feature flag retrieved from cache: ${key}`, "FeatureFlagService");
	...
	): Promise<{ data: FeatureFlag[]; total: number; page: number; limit: number }> {
		try {
			validateMinMaxRange(queryDto.minRolloutPercentage, queryDto.maxRolloutPercentage, "rollout percentage");
			const pagination = resolvePagination(queryDto, { page: 1, limit: 50 });
			const [data, total] = await Promise.all([
				this.featureFlagRepository.findFeatureFlags(queryDto, pagination),
				this.featureFlagRepository.countFeatureFlags(queryDto),
			]);
	...
	async isFeatureEnabled(key: string, userId?: string): Promise<boolean> {
		try {
			...
			if (!flag) {
				this.logger.log(`Feature flag not found: ${key} - defaulting to false`, "FeatureFlagService");
				return false;
			}
