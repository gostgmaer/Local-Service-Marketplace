...
export class FeatureFlagService {
  ...
  async createFeatureFlag(
    createFlagDto: CreateFeatureFlagDto,
  ): Promise<FeatureFlag> {
    try {
      const flag =
        await this.featureFlagRepository.createFeatureFlag(createFlagDto);
  ...
  async getAllFeatureFlags(
    ...
    try {
      validateMinMaxRange(
        queryDto.minRolloutPercentage,
        queryDto.maxRolloutPercentage,
        "rollout percentage",
      );
      ...
      this.logger.log(
        `Retrieved ${data.length} feature flags`,
        "FeatureFlagService",
      );
  ...
  async updateFeatureFlag(
    ...
    try {
      const flag = await this.featureFlagRepository.updateFeatureFlag(
        key,
        updateFlagDto,
      );
