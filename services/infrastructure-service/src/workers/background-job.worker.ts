...
export class BackgroundJobWorker extends WorkerHost implements OnModuleInit {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(
      `Processing background job ${jobId} of type ${jobType}`,
      "BackgroundJobWorker",
    );
    ...
    try {
      ...
      this.logger.error(
        `Background job ${jobId} failed: ${error.message}`,
        error.stack,
        "BackgroundJobWorker",
      );
  ...
  private async executeJobByType(jobType: string, data: any): Promise<void> {
    switch (jobType) {
      case "send-email": {
        ...
        this.logger.log(
          `send-email job delegated to comms-service for ${data.to}`,
          "BackgroundJobWorker",
        );
        break;
      }
      ...
      case "recalculate-metrics":
        ...
        this.logger.log(
          "recalculate-metrics: delegated to oversight-service (no-op in infra)",
          "BackgroundJobWorker",
        );
        break;
      default:
        this.logger.warn(
          `Unknown job type: ${jobType} — skipping`,
          "BackgroundJobWorker",
        );
    }
  }
