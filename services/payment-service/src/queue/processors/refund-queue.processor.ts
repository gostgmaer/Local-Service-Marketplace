...
export class RefundQueueProcessor {
	...
	async handleRefund(job: Job<RefundJobData>): Promise<void> {
		...
		this.logger.log(`Processing refund ${refundId} for payment ${paymentId}`, "RefundQueueProcessor");

		try {
			// Load the payment to get the gateway + original transaction ID
			const payment = await this.paymentRepository.getPaymentById(paymentId);
			if (!payment) {
				throw new Error(`Payment ${paymentId} not found for refund ${refundId}`);
			}
			...
			if (!transactionId) {
				throw new Error(`Payment ${paymentId} has no transaction_id — cannot issue refund via ${gateway}`);
			}

			// Call the real gateway adapter to issue the refund
			const refundResult = await this.paymentGateway.refundWith(gateway, { transactionId, amount, reason });
