...
export class MethodExpiryWorker extends WorkerHost implements OnModuleInit {
  constructor(
    ...
    @InjectQueue("payment.method-expiry")
    private readonly methodExpiryQueue: Queue,
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      "Payment method expiry repeatable job registered",
      "MethodExpiryWorker",
    );
  }
  ...
  private async handleCheckExpiringMethods(): Promise<void> {
    this.logger.log(
      "Checking for expiring payment methods",
      "MethodExpiryWorker",
    );
    ...
    if (expiringMethods.length === 0) {
      this.logger.log(
        "No expiring payment methods found",
        "MethodExpiryWorker",
      );
      return;
    }
    ...
    for (const [userId, methods] of methodsByUser.entries()) {
      ...
      if (!destinationEmail) {
        this.logger.warn(
          `Skipping expiry notification for user ${userId}: no billing email`,
          "MethodExpiryWorker",
        );
        continue;
      }
      ...
      await this.notificationClient.sendEmail({
        ...
        variables: {
          ...
          updatedFields: maskedMethods.map(
            (m) =>
              `${m} expires ${earliest.expiry_month}/${earliest.expiry_year} — please update`,
          ),
    ...
    this.logger.log(
      `Payment method expiry: notified ${notifiedUsers} users`,
      "MethodExpiryWorker",
    );
  }
