  ...
  implements OnModuleInit
{
      ...
      switch (job.name) {
        case "notify-payment-completed":
          return this.handlePaymentCompleted(
            job as Job<NotifyPaymentCompletedJobData>,
          );
        case "notify-refund-processed":
          return this.handleRefundProcessed(
            job as Job<NotifyRefundProcessedJobData>,
          );
        case "notify-payment-failed":
          return this.handlePaymentFailed(
            job as Job<NotifyPaymentFailedJobData>,
          );
  ...
  private async handlePaymentCompleted(
    job: Job<NotifyPaymentCompletedJobData>,
  ): Promise<void> {
    ...
    this.logger.log(
      `Payment confirmation sent to user ${userId}`,
      "PaymentNotificationWorker",
    );
  }

  private async handleRefundProcessed(
    job: Job<NotifyRefundProcessedJobData>,
  ): Promise<void> {
    ...
    this.logger.log(
      `Refund notification sent to user ${userId}`,
      "PaymentNotificationWorker",
    );
  }

  private async handlePaymentFailed(
    job: Job<NotifyPaymentFailedJobData>,
  ): Promise<void> {
    ...
    this.logger.log(
      `Payment failure notification sent to user ${userId}`,
      "PaymentNotificationWorker",
    );
  }

  private async handleCardSaved(
    job: Job<NotifyCardSavedJobData>,
  ): Promise<void> {
    ...
    await this.notificationClient.sendEmail({
      ...
      variables: {
        username: username || userEmail.split("@")[0],
        updatedFields: [
          `Payment method added: ${cardBrand} ending ${lastFour}`,
        ],
        updatedAt: new Date().toISOString(),
      },
    });
    this.logger.log(
      `Card-saved notification sent to user ${userId}`,
      "PaymentNotificationWorker",
    );
  }

  private async handleCardDeleted(
    job: Job<NotifyCardDeletedJobData>,
  ): Promise<void> {
    const { userId, cardBrand, lastFour } = job.data;
    const userEmail = await this.userClient.getUserEmail(userId);
    if (!userEmail) return;
    ...
    await this.notificationClient.sendEmail({
      ...
      variables: {
        username: username || userEmail.split("@")[0],
        updatedFields: [
          `Payment method removed: ${cardBrand} ending ${lastFour}`,
        ],
        updatedAt: new Date().toISOString(),
      },
    });
    this.logger.log(
      `Card-deleted notification sent to user ${userId}`,
      "PaymentNotificationWorker",
    );
  }
