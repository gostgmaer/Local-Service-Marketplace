...
export class SubscriptionWorker extends WorkerHost implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      "Subscription repeatable jobs registered",
      "SubscriptionWorker",
    );
  }
  ...
  private async handleCheckExpiringSubscriptions(): Promise<void> {
    ...
    this.logger.log(
      `Found ${expiring.length} expiring subscriptions`,
      "SubscriptionWorker",
    );
    ...
    for (const subscription of expiring) {
      ...
      const email = await this.userClient
        .getUserEmail(subscription.provider_id)
        .catch(() => null);
      if (!email) {
        this.logger.warn(
          `Cannot resolve email for provider ${subscription.provider_id} — skipping expiry notification`,
          "SubscriptionWorker",
        );
        continue;
      }
      ...
      await this.notificationClient.sendEmail({
        ...
        variables: {
          ...
          renewalDate: new Date(subscription.expires_at).toLocaleDateString(
            "en-IN",
          ),
  ...
  private async handleRenewSubscriptions(): Promise<void> {
    ...
    const renewals =
      await this.subscriptionRepository.getSubscriptionsForRenewal(24);
    this.logger.log(
      `Found ${renewals.length} subscriptions due for renewal`,
      "SubscriptionWorker",
    );
    ...
    for (const subscription of renewals) {
      try {
        ...
        if (!amount || amount <= 0) {
          this.logger.warn(
            `Subscription ${subscription.id} has invalid plan price — skipping`,
            "SubscriptionWorker",
          );
          continue;
        }
        ...
        if (chargeResult.status !== "succeeded") {
          this.logger.warn(
            `Renewal charge did not succeed for subscription ${subscription.id} (status: ${chargeResult.status})`,
            "SubscriptionWorker",
          );
          continue;
        }
        ...
        this.logger.log(
          `Renewed subscription ${subscription.id} until ${newExpiresAt.toISOString()}`,
          "SubscriptionWorker",
        );
        ...
        if (email) {
          await this.notificationClient
            ...
            .catch(() => {
              /* non-critical: log already done by notificationClient */
            });
        }
      } catch (error: any) {
        this.logger.error(
  ...
  private async handleNotifySubscriptionExpiring(data: any): Promise<void> {
    ...
    const email = await this.userClient
      .getUserEmail(providerId)
      .catch(() => null);
    if (!email) {
      this.logger.warn(
        `Cannot resolve email for provider ${providerId} — skipping expiry notification`,
        "SubscriptionWorker",
      );
      return;
    }
  ...
  private async handleNotifySubscriptionCancelled(data: any): Promise<void> {
    ...
    if (!email) {
      this.logger.warn(
        `Cannot resolve email for provider ${providerId} — skipping cancellation notification`,
        "SubscriptionWorker",
      );
      return;
    }
  ...
  private async handleNotifySubscriptionActivated(data: any): Promise<void> {
    ...
    if (!email) {
      this.logger.warn(
        `Cannot resolve email for provider ${providerId} — skipping activation notification`,
        "SubscriptionWorker",
      );
      return;
    }
