	...
	async checkExpiringPaymentMethods(): Promise<void> {
		...
		try {
			...
			for (const [userId, methods] of methodsByUser.entries()) {
				const destinationEmail = methods.find((method) => !!method.billing_email)?.billing_email;
				if (!destinationEmail) {
					this.logger.warn(`Skipping expiry notification for user ${userId}: no billing email available`);
					continue;
				}

				const maskedMethods = methods.map(
					(method) => `${method.card_brand || "card"} ****${method.last_four || "----"}`,
				);
			...
			this.logger.error(`Error checking expiring payment methods: ${error.message}`, error.stack);
		}
	}
}
