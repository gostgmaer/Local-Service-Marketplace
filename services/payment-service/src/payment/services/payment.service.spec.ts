...
const makeGateway = () =>
	({
		charge: jest.fn().mockResolvedValue({ transactionId: "txn_mock_123", status: "succeeded" }),
		chargeWith: jest.fn().mockResolvedValue({ transactionId: "txn_mock_123", status: "succeeded" }),
...
describe("PaymentService.createPayment", () => {
	...
	const createService = (
		...
		return { service, kafka, analytics, paymentRepository, couponService, gateway };
	};

	it("creates payment without coupon and tracks analytics", async () => {
		const { service, kafka, analytics, paymentRepository, gateway } = createService();

		await service.createPayment("job-1", 100, "USD", "user-1", "prov-1");

		expect(gateway.charge).toHaveBeenCalledWith(expect.objectContaining({ amount: 100, currency: "USD" }));
		...
		expect(paymentRepository.updatePaymentStatus).toHaveBeenCalledWith("pay-1", "completed", "txn_mock_123");
