...
export class SavedPaymentMethodService {
   ...
   */
  async savePaymentMethod(dto: SavePaymentMethodDto): Promise<SavedPaymentMethod> {
    // Validate gateway token
    if (!dto.gateway_payment_method_id) {
      throw new BadRequestException('Payment method ID from gateway is required');
    }
    ...
    if (dto.payment_type === 'card') {
      ...
      if (dto.expiry_year) {
        ...
        if (dto.expiry_year < currentYear || dto.expiry_year > currentYear + 20) {
    ...
    const existingMethods = await this.paymentMethodRepository.findByUserId(dto.user_id);
   ...
   */
  async getExpiringCards(userId: string, monthsAhead: number = 2): Promise<SavedPaymentMethod[]> {
    ...
    return methods.filter(method => {
      if (method.payment_type !== 'card' || !method.expiry_month || !method.expiry_year) {
        return false;
      }
