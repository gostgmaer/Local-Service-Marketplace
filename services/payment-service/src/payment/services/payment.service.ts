...
export class PaymentService {
  ...
  async createPayment(
    ...
    const activeGatewayName =
      gateway ?? this.paymentGateway.getActiveGatewayName();
    ...
    this.logger.log(
      `Payment created: ${payment.id}, gateway status: ${chargeResult.status}`,
      "PaymentService",
    );
  ...
  async updatePaymentStatus(
    ...
    this.logger.log(
      `Updating payment ${id} status to ${status}`,
      "PaymentService",
    );
  ...
  async getPaymentsByUserPaginated(
    ...
    if (queryDto.cursor) {
      const payments = await this.paymentRepository.getPaymentsByUserPaginated(
        userId,
        queryDto,
      );
      ...
      const nextCursor =
        hasMore && data.length > 0
          ? (data[data.length - 1].created_at as any)
          : undefined;
  ...
  async getProviderEarnings(
    ...
    this.logger.log(
      `Fetching earnings for provider ${providerId}`,
      "PaymentService",
    );
  ...
  async getProviderTransactions(
    ...
    this.logger.log(
      `Fetching transactions for provider ${providerId}`,
      "PaymentService",
    );
    ...
    const userIds = [
      ...new Set(result.data.map((t: any) => t.user_id).filter(Boolean)),
    ];
  ...
  async getProviderPayouts(providerId: string): Promise<any[]> {
    this.logger.log(
      `Fetching payouts for provider ${providerId}`,
      "PaymentService",
    );
