...
export class PaymentRepository {
	...
	async getPaymentsByUserPaginated(userId: string, queryDto: TransactionQueryDto): Promise<Payment[]> {
		const {
			limit = 20,
			page,
			cursor,
	...
	async getPaymentsByPaymentMethod(paymentMethod: string, limit: number = 50): Promise<Payment[]> {
		const query = `
      SELECT * FROM payments
    ...
    `;
		...
		return (
			result.rows[0] || { total_earnings: 0, total_paid: 0, pending_payout: 0, completed_count: 0, currency: "USD" }
		);
	}

	async getProviderEarningsByMonth(providerId: string, startDate?: Date, endDate?: Date): Promise<any[]> {
		const query = `
      SELECT 
	...
	async getProviderTransactions(providerId: string, queryDto: TransactionQueryDto): Promise<any> {
		const {
			limit = 20,
			page,
			cursor,
    ...
    `;
		...
		return {
			data,
			total: usingOffset ? await this.countProviderTransactions(providerId, queryDto) : data.length,
			nextCursor,
			hasMore,
		};
	}

	async countProviderTransactions(providerId: string, queryDto: TransactionQueryDto): Promise<number> {
		const { status, payment_method, created_from, created_to } = queryDto;
