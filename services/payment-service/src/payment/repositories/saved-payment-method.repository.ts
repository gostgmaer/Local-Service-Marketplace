...
export class SavedPaymentMethodRepository {
	...
	async create(data: SavePaymentMethodDto): Promise<SavedPaymentMethod> {
		...
		if (data.is_default) {
			await this.pool.query("UPDATE saved_payment_methods SET is_default = false WHERE user_id = $1", [data.user_id]);
		}
	...
	async findExpiringWithinMonths(monthWindow = 1): Promise<SavedPaymentMethod[]> {
		const query = `
      SELECT *
