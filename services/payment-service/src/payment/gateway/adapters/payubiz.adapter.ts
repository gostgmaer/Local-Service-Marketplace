...
export class PayUbizAdapter implements IGatewayAdapter {
	...
	constructor(private readonly configService: ConfigService) {
		const apiUrl = this.configService.get<string>("PAYU_API_URL", "https://test.payu.in");
	...
	async refund(params: RefundParams): Promise<RefundResult> {
		...
		const response = await this.http.post("/merchant/postservice.php?form=2", body.toString(), {
		...
		const refundId = data?.mihpayid ?? data?.refundId ?? `ref_payu_${Date.now()}`;
	...
	async getPaymentStatus(transactionId: string): Promise<ChargeResult> {
		...
		const body = new URLSearchParams({ key, command, var1: transactionId, hash });
		const response = await this.http.post("/merchant/postservice.php?form=2", body.toString(), {
			headers: { "Content-Type": "application/x-www-form-urlencoded" },
		});
		...
		const normalized =
			payuStatus === "success" ? "succeeded"
			: payuStatus === "failure" ? "failed"
			: "pending";
	...
	verifyWebhookSignature(rawBody: Buffer, headers: Record<string, string>): boolean {
		const salt = this.configService.get<string>("PAYU_SALT", "");
		if (!salt) {
			this.logger.warn("PAYU_SALT not set — skipping PayU webhook verification");
			return true;
		}
	...
	parseWebhookEvent(payload: Record<string, any>): ParsedWebhookEvent {
		...
		if (status === "failure" || status === "failed") {
			return { eventType: "payment.failed", transactionId: txnid, paymentId: payload.udf1 ?? undefined };
		}
