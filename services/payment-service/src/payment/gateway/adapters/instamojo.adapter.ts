...
export class InstamojoAdapter implements IGatewayAdapter {
	...
	constructor(private readonly configService: ConfigService) {
		const apiUrl = this.configService.get<string>("INSTAMOJO_API_URL", "https://test.instamojo.com");
		...
		this.http = axios.create({
			baseURL: apiUrl,
			headers: { "X-Api-Key": apiKey, "X-Auth-Token": authToken, "Content-Type": "application/json" },
		});
		this.logger.log(`InstamojoAdapter initialised (${apiUrl})`);
	}

	async charge(params: ChargeParams): Promise<ChargeResult> {
		const webhookUrl = this.configService.get<string>("INSTAMOJO_WEBHOOK_URL", "");
		const redirectUrl = this.configService.get<string>("INSTAMOJO_REDIRECT_URL", "");
	...
	async getPaymentStatus(transactionId: string): Promise<ChargeResult> {
		...
		const response = await this.http.get(`/api/1.1/payment-requests/${transactionId}/`);
		const pr = response.data?.payment_request as Record<string, any>;
		const payments: any[] = pr?.payments ?? [];

		// Check if any linked payment is completed
		const completed = payments.find((p: any) => (p.status ?? "").toLowerCase() === "credit");
	...
	verifyWebhookSignature(rawBody: Buffer, _headers: Record<string, string>): boolean {
		...
		if (!salt) {
			this.logger.warn("INSTAMOJO_SALT not set — skipping webhook verification");
			return true;
		}
		...
		try {
			...
			const expectedMac = crypto.createHmac("sha1", salt).update(message).digest("hex");
	...
	parseWebhookEvent(payload: Record<string, any>): ParsedWebhookEvent {
		...
		if (status === "failed" || status === "failure") {
			return { eventType: "payment.failed", transactionId: payload.payment_request_id };
		}
