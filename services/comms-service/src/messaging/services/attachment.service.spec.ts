...
describe('AttachmentService', () => {
  ...
  describe('createAttachment', () => {
    it('should create and return a new attachment', async () => {
      ...
      const result = await service.createAttachment(
        'msg-uuid-1', 'https://cdn.example.com/file.pdf', 'file.pdf', 1024, 'application/pdf',
      );
    ...
    it('should create attachment with only required fields', async () => {
      const minimal = { ...mockAttachment, file_name: undefined, file_size: undefined, mime_type: undefined };
      mockAttachmentRepo.createAttachment.mockResolvedValue(minimal);
      const result = await service.createAttachment('msg-uuid-1', 'https://cdn.example.com/file.pdf');
  ...
  describe('getAttachmentById', () => {
    ...
    it('should throw NotFoundException when not found', async () => {
      ...
      await expect(service.getAttachmentById('missing')).rejects.toThrow(NotFoundException);
    });
  });

  describe('getAttachmentsByMessageId', () => {
    it('should return attachments for a message', async () => {
      mockAttachmentRepo.getAttachmentsByMessageId.mockResolvedValue([mockAttachment]);
  ...
  describe('deleteAttachment', () => {
    it('should delete an existing attachment', async () => {
      ...
      expect(mockAttachmentRepo.deleteAttachment).toHaveBeenCalledWith('att-uuid-1');
    });

    it('should throw NotFoundException when attachment not found', async () => {
      mockAttachmentRepo.getAttachmentById.mockResolvedValue(null);
      await expect(service.deleteAttachment('missing')).rejects.toThrow(NotFoundException);
    });
  });
});
