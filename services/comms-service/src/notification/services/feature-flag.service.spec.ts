...
describe('FeatureFlagService', () => {
  ...
  function createService(envOverrides: Record<string, string> = {}): FeatureFlagService {
  ...
  describe('checkFeatureOrThrow', () => {
    ...
    it('should throw BadRequestException when feature is disabled', () => {
      const svc = createService({});
      expect(() => svc.checkFeatureOrThrow('sms', 'SMS')).toThrow(BadRequestException);
    });

    it('should include feature name in error message', () => {
      const svc = createService({});
      expect(() => svc.checkFeatureOrThrow('push', 'Push notifications')).toThrow(
        /Push notifications is disabled/,
      );
    });
  });
});
