...
describe('NotificationPreferencesService', () => {
  ...
  describe('updatePreferences', () => {
    it('should update preferences', async () => {
      ...
      const result = await service.updatePreferences('user-uuid-1', { marketing_emails: true });
      expect(mockRepo.update).toHaveBeenCalledWith('pref-uuid-1', { marketing_emails: true });
  ...
  describe('checkNotificationEnabled', () => {
    it('should return true for enabled notification type', async () => {
      mockRepo.findByUserId.mockResolvedValue(mockPrefs);
      const result = await service.checkNotificationEnabled('user-uuid-1', 'email_notifications');
      expect(result).toBe(true);
    });

    it('should return false for disabled notification type', async () => {
      mockRepo.findByUserId.mockResolvedValue(mockPrefs);
      const result = await service.checkNotificationEnabled('user-uuid-1', 'marketing_emails');
  ...
  describe('getUsersForNotification', () => {
    it('should return users with the preference enabled', async () => {
      ...
      const result = await service.getUsersForNotification('email_notifications');
      expect(mockRepo.getUsersWithPreference).toHaveBeenCalledWith('email_notifications', true);
  ...
  describe('disableAllNotifications', () => {
    it('should set all notification flags to false', async () => {
      mockRepo.findByUserId.mockResolvedValue(mockPrefs);
      const disabled = { ...mockPrefs, email_notifications: false, sms_notifications: false };
