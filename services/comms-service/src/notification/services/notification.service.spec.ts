...
describe('NotificationService', () => {
  ...
  describe('createNotification', () => {
    it('should create notification and queue email', async () => {
      ...
      const result = await service.createNotification('user-uuid-1', 'request_created', 'Your request was created');

      expect(result).toEqual(mockNotification);
      expect(mockNotifRepo.createNotification).toHaveBeenCalledWith('user-uuid-1', 'request_created', 'Your request was created');
      expect(mockDeliveryRepo.createDelivery).toHaveBeenCalledTimes(2); // email + push
      expect(mockEmailQueue.add).toHaveBeenCalledWith(
        'send-email',
        expect.objectContaining({ userId: 'user-uuid-1', type: 'request_created' }),
  ...
  describe('getNotificationById', () => {
    ...
    it('should throw ForbiddenException when userId does not match', async () => {
      mockNotifRepo.getNotificationById.mockResolvedValue(mockNotification);
      await expect(service.getNotificationById('notif-uuid-1', 'other-user')).rejects.toThrow(ForbiddenException);
    });

    it('should allow access when userId matches', async () => {
      mockNotifRepo.getNotificationById.mockResolvedValue(mockNotification);
      const result = await service.getNotificationById('notif-uuid-1', 'user-uuid-1');
      expect(result).toEqual(mockNotification);
    });
  });
  ...
  describe('getNotificationsByUserId', () => {
    it('should return notifications list', async () => {
      mockNotifRepo.getNotificationsByUserId.mockResolvedValue([mockNotification]);
    ...
    it('should pass limit parameter', async () => {
      ...
      expect(mockNotifRepo.getNotificationsByUserId).toHaveBeenCalledWith('user-uuid-1', 10);
    });
  });
  ...
  describe('deleteNotification', () => {
    it('should delete owned notification', async () => {
      ...
      expect(mockNotifRepo.deleteNotification).toHaveBeenCalledWith('notif-uuid-1');
    });

    it('should throw ForbiddenException if not owner', async () => {
      mockNotifRepo.getNotificationById.mockResolvedValue(mockNotification);
      await expect(service.deleteNotification('notif-uuid-1', 'other-user')).rejects.toThrow(ForbiddenException);
    });
  });

  describe('sendNotification', () => {
    it('should send email when channel is EMAIL', async () => {
      mockEmailClient.sendEmail.mockResolvedValue({ success: true, messageId: 'mid-1' });
  ...
  describe('sendEmailDirect', () => {
    ...
    it('should throw when email feature is disabled', async () => {
      ...
      Object.defineProperty(mockFeatureFlags, 'emailEnabled', { value: false, writable: true });
      await expect(service.sendEmailDirect({
        to: 'user@example.com',
        subject: 'Test',
        message: 'Hello',
      })).rejects.toThrow(BadRequestException);
      Object.defineProperty(mockFeatureFlags, 'emailEnabled', { value: true, writable: true });
    });
  });
});
