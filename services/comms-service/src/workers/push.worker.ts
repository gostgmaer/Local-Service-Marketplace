...
export class PushWorker extends WorkerHost implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      "PushWorker: repeatable retry job registered",
      "PushWorker",
    );
  }
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    switch (job.name) {
      ...
      default:
        this.logger.warn(
          `PushWorker: unknown job name "${job.name}"`,
          "PushWorker",
        );
    }
  }
  ...
  private async handleDeliverPush(job: Job<DeliverPushJobData>): Promise<void> {
    ...
    try {
      ...
      if (success) {
        ...
        this.logger.log(
          `PushWorker: delivery ${deliveryId} sent`,
          "PushWorker",
        );
      } else {
  ...
  private async handleRetryPushWorker(): Promise<void> {
    this.logger.log(
      "PushWorker: scanning for pending push deliveries...",
      "PushWorker",
    );
