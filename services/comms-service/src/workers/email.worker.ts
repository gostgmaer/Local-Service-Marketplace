...
export class EmailWorker extends WorkerHost implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      "EmailWorker: repeatable retry job registered",
      "EmailWorker",
    );
  }
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    switch (job.name) {
      ...
      default:
        this.logger.warn(
          `EmailWorker: unknown job name "${job.name}"`,
          "EmailWorker",
        );
    }
  }
  ...
  private async handleDeliverEmail(
    job: Job<DeliverEmailJobData>,
  ): Promise<void> {
    const {
      deliveryId,
      userId,
        ...
        variables: variables || {
          ...
          replyUrl:
            (appContext?.appUrl ||
              ...
              "http://localhost:3000") +
            (appContext?.ctaPath || "/notifications"),
        },
        appContext,
      });
  ...
  private async handleRetryEmailWorker(): Promise<void> {
    this.logger.log(
      "EmailWorker: scanning for pending email deliveries...",
      "EmailWorker",
    );
    ...
    if (emailDeliveries.length === 0) {
      this.logger.log(
        "EmailWorker: no pending email deliveries found",
        "EmailWorker",
      );
      return;
    }
  ...
  onCompleted(job: Job): void {
    this.logger.log(`Job "${job.name}/${job.id}" completed`, "EmailWorker");
  }
