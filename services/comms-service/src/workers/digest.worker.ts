...
export class DigestWorker extends WorkerHost implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      "DigestWorker: daily digest job registered",
      "DigestWorker",
    );
  }
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    try {
      switch (job.name) {
        ...
        default:
          this.logger.warn(
            `DigestWorker: unknown job name "${job.name}"`,
            "DigestWorker",
          );
      }
    } catch (error: any) {
  ...
  private async handleSendUnreadDigest(): Promise<void> {
    this.logger.log(
      "DigestWorker: building daily unread digest...",
      "DigestWorker",
    );

    // Fetch users with unread notifications (limit to avoid memory spike)
    const unreadGroups =
      await this.notificationRepository.getUsersWithUnreadNotifications(100);

    if (!unreadGroups || unreadGroups.length === 0) {
      this.logger.log(
        "DigestWorker: no unread notifications to digest",
        "DigestWorker",
      );
      return;
    }
    ...
    this.logger.log(
      `DigestWorker: digest sent to ${sent}/${unreadGroups.length} users`,
      "DigestWorker",
    );
  }
