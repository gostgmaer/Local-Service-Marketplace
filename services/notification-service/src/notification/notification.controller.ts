...
export class NotificationController {
  ...
  async getNotifications(
    ...
    if (!this.featureFlags.inAppNotificationsEnabled) {
      throw new BadRequestException('In-app notifications are disabled. Set IN_APP_NOTIFICATIONS_ENABLED=true to enable this feature.');
    }
  ...
  async processPush() {
    ...
    if (!this.featureFlags.pushNotificationsEnabled) {
      throw new BadRequestException('Push notifications are disabled. Set PUSH_NOTIFICATIONS_ENABLED=true to enable this feature.');
    }
