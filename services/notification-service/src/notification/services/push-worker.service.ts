...
export class PushWorkerService {
  ...
  async processPendingPushNotifications(): Promise<void> {
    this.logger.log('Processing pending push notifications', 'PushWorkerService');

    try {
      const pendingDeliveries = await this.deliveryRepository.getPendingDeliveries();
      const pushDeliveries = pendingDeliveries.filter((d) => d.channel === 'push');

      this.logger.log(`Found ${pushDeliveries.length} pending push deliveries`, 'PushWorkerService');
      ...
      this.logger.log('Push notification processing completed', 'PushWorkerService');
    } catch (error) {
      this.logger.error(`Push notification processing failed: ${error.message}`, 'PushWorkerService');
    }
  }

  private async sendPushNotification(userId: string, type: string, message: string): Promise<void> {
    ...
    this.logger.log(`Push notification sent to user ${userId} with type ${type}`, 'PushWorkerService');
  }
}
