...
export class EmailWorkerService {
  ...
  async processPendingEmails(): Promise<void> {
    this.logger.log('Processing pending email notifications', 'EmailWorkerService');
    ...
    try {
      ...
      this.logger.log(`Found ${emailDeliveries.length} pending email deliveries`, 'EmailWorkerService');
      ...
      this.logger.error(`Email processing failed: ${error.message}`, 'EmailWorkerService');
    }
  }
  ...
  private async sendEmail(userId: string, type: string, message: string): Promise<void> {
    ...
    this.logger.log(`Email sent to user ${userId} with type ${type}`, 'EmailWorkerService');
  }
}
