...
describe("AuthService.login", () => {
  it("returns tokens on successful login", async () => {
    ...
    const { service, userRepo, sessionRepo, loginAttemptRepo, jwtService } =
      makeService({
        userRepo: {
...
describe("AuthService.register", () => {
  ...
  it("throws BadRequestException when neither email nor phone is provided", async () => {
    const { service } = makeService();
    const { BadRequestException: BE } =
      await import("@/common/exceptions/http.exceptions");

    await expect(service.register({ name: "Alice" } as any)).rejects.toThrow(
      BE,
    );
  });
});
