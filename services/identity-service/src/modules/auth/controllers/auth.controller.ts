...
export class AuthController {
  ...
  async checkIdentifier(
    ...
    const otpAvailable = this.authService.isOtpServiceAvailable(
      checkIdentifierDto.type,
    );
  ...
  async useBackupCode(
    ...
    const valid = await this.authService.useBackupCode(
      req.user["sub"],
      dto.backupCode,
    );
  ...
  async resendVerificationEmail(
    ...
    return {
      result: "Verification email sent if email is registered and not verified",
    };
  }
  ...
  async requestAccountDeletion(
    ...
    return {
      result: "Account deletion requested. You have 30 days to cancel.",
    };
  }
  ...
  async appleAuthCallback(
    ...
    this.logger.info("Apple OAuth callback", {
      context: "AuthController",
      email: oauthUser.email,
      ipAddress,
    });
