...
export class ProviderPortfolioService {
	constructor(private readonly portfolioRepository: ProviderPortfolioRepository) {}
	...
	async createPortfolioItem(
		...
		if (imageUrls.length > 10) {
			throw new BadRequestException("Maximum 10 images allowed per portfolio item");
		}

		// Get the current max display_order for this provider
		const existingItems = await this.portfolioRepository.findByProvider(providerId);
		const maxOrder = existingItems.length > 0 ? Math.max(...existingItems.map((item) => item.display_order)) : 0;
	...
	async getProviderPortfolio(providerId: string): Promise<{ data: ProviderPortfolio[]; total: number }> {
		const data = await this.portfolioRepository.findByProvider(providerId);
		return { data, total: data.length };
	}

	async getPortfolioItemById(itemId: string, userId?: string): Promise<ProviderPortfolio> {
	...
	async reorderPortfolio(providerId: string, userId: string, orderedIds: string[]): Promise<ProviderPortfolio[]> {
		// Authorization: verify user owns this provider
		...
		if (invalidIds.length > 0) {
			throw new BadRequestException(`Invalid portfolio item IDs: ${invalidIds.join(", ")}`);
		}

		// Create itemOrders array with order indices
		const itemOrders = orderedIds.map((id, index) => ({ id, order: index + 1 }));
