...
export class ProviderDocumentService {
	constructor(private readonly documentRepository: ProviderDocumentRepository) {}
	...
	async verifyDocument(documentId: string, adminUserId: string, dto: VerifyDocumentDto): Promise<ProviderDocument> {
	...
	async getProviderDocuments(providerId: string): Promise<{ data: ProviderDocument[]; total: number }> {
		const data = await this.documentRepository.findByProvider(providerId);
	...
	async getPendingDocuments(): Promise<{ data: ProviderDocument[]; total: number }> {
	...
	async getExpiringDocuments(days: number = 30): Promise<{ data: ProviderDocument[]; total: number }> {
		// Admin only - get documents expiring within X days
		const data = await this.documentRepository.getExpiringDocuments(days);
		return { data, total: data.length };
	}
	...
	async checkProviderVerificationStatus(
		providerId: string,
	): Promise<{
		...
		const requiredDocumentTypes = ["government_id", "business_license"];

		const verifiedCount = documents.filter((d) => d.verified).length;
		const pendingCount = documents.filter((d) => !d.verified).length;

		const uploadedTypes = documents.map((d) => d.document_type);
		const missingTypes = requiredDocumentTypes.filter((type) => !uploadedTypes.includes(type as any));

		const fullyVerified =
			missingTypes.length === 0 &&
			requiredDocumentTypes.every((type) => documents.some((d) => d.document_type === type && d.verified));
