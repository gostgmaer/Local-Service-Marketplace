...
export class FavoriteService {
	...
	async saveFavorite(createFavoriteDto: CreateFavoriteDto): Promise<any> {
		...
		this.logger.info("Saving favorite provider", { context: "FavoriteService", user_id, provider_id });
		...
		this.logger.info("Favorite saved successfully", { context: "FavoriteService", favorite_id: favorite.id });
	...
	async getFavorites(userId: string): Promise<{ data: any[]; total: number }> {
		this.logger.info("Fetching user favorites", { context: "FavoriteService", user_id: userId });
