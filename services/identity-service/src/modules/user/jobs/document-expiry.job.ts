  ...
  async checkExpiringDocuments(): Promise<void> {
    ...
    try {
      ...
      const expiringDocuments =
        await this.documentRepository.getExpiringDocuments(30);

      console.log(
        `[DocumentExpiryJob] Found ${expiringDocuments.length} expiring documents`,
      );
      ...
      for (const doc of expiringDocuments) {
        ...
        console.log(
          `[DocumentExpiryJob] Notification sent for document ${doc.id}`,
        );
      }
      ...
      console.error(
        "[DocumentExpiryJob] Error checking expiring documents:",
        error,
      );
    }
  }
  ...
  async checkExpiredDocuments(): Promise<void> {
    ...
    try {
      ...
      const recentlyExpired =
        await this.documentRepository.getExpiringDocuments(-7);

      console.log(
        `[DocumentExpiryJob] Found ${recentlyExpired.length} recently expired documents`,
      );
      ...
      for (const doc of recentlyExpired) {
        ...
        console.log(
          `[DocumentExpiryJob] Urgent notification sent for expired document ${doc.id}`,
        );
      }
      ...
      console.error(
        "[DocumentExpiryJob] Error checking expired documents:",
        error,
      );
    }
  }
