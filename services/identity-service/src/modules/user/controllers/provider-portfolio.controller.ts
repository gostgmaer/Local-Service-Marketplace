...
export class ProviderPortfolioController {
	...
	async createPortfolioItem(
		...
		return { success: true, data: portfolioItem, message: "Portfolio item created successfully" };
	}
	...
	async getPortfolioItem(@Param("itemId", ParseUUIDPipe) itemId: string, @Request() req: any) {
		const item = await this.portfolioService.getPortfolioItemById(itemId, req.user?.userId);
