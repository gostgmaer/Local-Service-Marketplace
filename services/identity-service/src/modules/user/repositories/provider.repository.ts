...
export class ProviderRepository {
  ...
  async findByUserId(userId: string): Promise<Provider | null> {
    ...
    const query =
      "SELECT * FROM providers WHERE user_id = $1 AND deleted_at IS NULL";
  ...
  async findPaginated(
    ...
    [categoryId, userId] = await Promise.all([
      categoryId
        ? resolveId(this.pool, "service_categories", categoryId)
    ...
    if (cursor && !offset) {
      conditions.push(
        `providers.created_at < (SELECT created_at FROM providers WHERE id = $${paramCount})`,
      );
      values.push(cursor);
      paramCount++;
    }
