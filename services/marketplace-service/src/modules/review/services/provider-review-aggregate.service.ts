...
export class ProviderReviewAggregateService {
	constructor(private readonly aggregateRepository: ProviderReviewAggregateRepository) {}

	/**
	 * Update aggregate stats for a provider
	 * Called whenever a review is created, updated, or deleted
	 */
	async updateProviderAggregate(providerId: string): Promise<ProviderReviewAggregate> {
	 ...
	 */
	async getProviderAggregate(providerId: string): Promise<ProviderReviewAggregate> {
	 ...
	 */
	async getTopRatedProviders(limit: number = 10): Promise<ProviderReviewAggregate[]> {
	...
	async getProvidersByRating(
		...
		return this.aggregateRepository.findByRatingRange(minRating, maxRating, limit);
	}
	...
	async getRatingDistribution(
		...
		const distribution = [
			...
			...item,
			percentage: aggregate.total_reviews > 0 ? Math.round((item.count / aggregate.total_reviews) * 100) : 0,
		}));

		return { total_reviews: aggregate.total_reviews, average_rating: aggregate.average_rating, distribution };
	}
	 ...
	 */
	async checkTrustBadgeEligibility(
		providerId: string,
	): Promise<{
		eligible: boolean;
		total_reviews: number;
		...
		const eligible =
			aggregate.total_reviews >= minimumRequiredReviews && aggregate.average_rating >= minimumAverageRating;
