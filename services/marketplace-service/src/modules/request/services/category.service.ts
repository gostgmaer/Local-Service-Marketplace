...
export class CategoryService {
	...
	async getAllCategories(): Promise<{ data: ServiceCategory[]; total: number }> {
		...
		if (this.redisService.isCacheEnabled()) {
			...
			if (cached) {
				...
				const categories = JSON.parse(cached) as ServiceCategory[];
	...
	async searchCategories(searchTerm: string, limit: number = 10): Promise<{ data: ServiceCategory[]; total: number }> {
