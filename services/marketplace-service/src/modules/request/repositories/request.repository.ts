...
export class RequestRepository {
  ...
  async createRequest(dto: CreateRequestDto): Promise<ServiceRequest> {
    const [userId, categoryId] = await Promise.all([
      dto.user_id
        ? resolveId(this.pool, "users", dto.user_id)
  ...
  async getRequestsPaginated(
    ...
    [user_id, category_id] = await Promise.all([
      user_id
        ? resolveId(this.pool, "users", user_id)
        : Promise.resolve(undefined),
      category_id
        ? resolveId(this.pool, "service_categories", category_id)
        : Promise.resolve(undefined),
    ]);
    ...
    return result.rows.map((row) => {
      const request: ServiceRequest = {
        ...
        budget: row.budget != null ? parseFloat(row.budget) : 0,
        status: row.status,
  ...
  async getRequestsByPreferredDate(
    startDate: Date,
    endDate: Date,
    limit: number = 20,
  ): Promise<ServiceRequest[]> {
