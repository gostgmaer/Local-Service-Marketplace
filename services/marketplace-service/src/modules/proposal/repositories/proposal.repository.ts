...
export class ProposalRepository {
	...
	async withdrawProposal(id: string, providerId: string): Promise<Proposal | null> {
		const query = `
      UPDATE proposals
	...
	async getProposalsByDateRange(startDate: Date, endDate: Date, limit: number = 50): Promise<Proposal[]> {
		const query = `
	...
	async getProposalsByEstimatedHours(minHours: number, maxHours: number, limit: number = 20): Promise<Proposal[]> {
		const query = `
      SELECT * FROM proposals
