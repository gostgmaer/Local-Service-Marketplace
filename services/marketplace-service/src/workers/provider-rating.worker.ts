...
export class ProviderRatingWorker extends WorkerHost implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      "Provider rating repeatable jobs registered",
  ...
  private async handleFullAggregateRefresh(): Promise<void> {
    this.logger.log(
      "Starting full provider rating aggregate refresh",
      "ProviderRatingWorker",
    );
    const count = await this.aggregateRepository.refreshAllAggregates();
    this.logger.log(
      `Full aggregate refresh completed: ${count} providers updated`,
      "ProviderRatingWorker",
    );
  }
  ...
  private async handleQuickAggregateRefresh(): Promise<void> {
    this.logger.log(
      "Starting quick provider rating aggregate refresh",
      "ProviderRatingWorker",
    );
    const count = await this.aggregateRepository.refreshRecentAggregates();
    this.logger.log(
      `Quick aggregate refresh completed: ${count} providers updated`,
      "ProviderRatingWorker",
    );
  }

  private async handleRecalculateProviderRating(data: {
    providerId: string;
  }): Promise<void> {
    const { providerId } = data;
    this.logger.log(
      `Recalculating rating for provider ${providerId}`,
      "ProviderRatingWorker",
    );
