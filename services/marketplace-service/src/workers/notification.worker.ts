...
export class MarketplaceNotificationWorker
  extends WorkerHost
  implements OnModuleInit
{
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    try {
      switch (job.name) {
        case "notify-request-created":
          return this.handleRequestCreated(job.data);
        case "notify-request-updated":
          return this.handleRequestUpdated(job.data);
        case "notify-request-deleted":
          return this.handleRequestDeleted(job.data);
        case "notify-request-cancelled":
          return this.handleRequestCancelled(job.data);
        case "notify-proposal-submitted":
          return this.handleProposalSubmitted(job.data);
        case "notify-proposal-accepted":
          return this.handleProposalAccepted(job.data);
        case "notify-proposal-rejected":
          return this.handleProposalRejected(job.data);
        case "notify-job-assigned":
          return this.handleJobAssigned(job.data);
        case "notify-job-status-changed":
          return this.handleJobStatusChanged(job.data);
        case "notify-job-completed":
          return this.handleJobCompleted(job.data);
        case "notify-job-cancelled":
          return this.handleJobCancelled(job.data);
        case "notify-review-created":
          return this.handleReviewCreated(job.data);
        default:
  ...
  private async handleRequestCreated(data: any): Promise<void> {
    const { userId, guestEmail, requestId, description, budget, category } =
      data;
    const emailTo = userId
      ? await this.userClient.getUserEmail(userId)
      : guestEmail;
    if (!emailTo) return;
    const username = userId
      ? await this.userClient.getUserName(userId).catch(() => null)
      : null;
  ...
  private async handleRequestUpdated(data: any): Promise<void> {
    ...
    if (!emailTo) return;
    const username = await this.userClient
      .getUserName(userId)
      .catch(() => null);
    await this.notificationClient.sendEmail({
      to: emailTo,
  ...
  private async handleProposalAccepted(data: any): Promise<void> {
    const { providerId, customerId, requestId, proposalId, price, jobId } =
      data;
  ...
  private async handleProposalRejected(data: any): Promise<void> {
    ...
    const emailTo = await this.userClient.getUserEmail(providerId);
    if (!emailTo) return;
    const providerName = await this.userClient
