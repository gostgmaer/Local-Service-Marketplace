...
describe("AnalyticsService", () => {
  ...
  describe("trackActivity", () => {
    ...
    it("throws and logs on repository failure", async () => {
      ...
      userActivityRepository.trackActivity.mockRejectedValue(
        new Error("DB error"),
      );

      await expect(
        service.trackActivity({ user_id: "u1", action: "test" } as any),
  ...
  describe("getUserActivity", () => {
    it("returns paginated user activity", async () => {
      ...
      expect(userActivityRepository.getUserActivity).toHaveBeenCalledWith(
        "user-1",
        10,
        0,
      );
  ...
  describe("getAllActivity", () => {
    it("returns all activity without user filter", async () => {
      const { service, userActivityRepository } = createService();
      userActivityRepository.getAllActivity.mockResolvedValue([
        { id: "a1" },
        { id: "a2" },
      ]);
