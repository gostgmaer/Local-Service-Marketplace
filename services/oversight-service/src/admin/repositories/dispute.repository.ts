...
import {
  DisputeListQueryDto,
  DisputeSortBy,
} from "../dto/dispute-list-query.dto";
...
export class DisputeRepository {
  ...
  async getUserDisputes(
    ...
    const conditions: string[] = [
      "(opened_by = $1 OR job_id IN (SELECT id FROM jobs WHERE customer_id = $1 OR provider_id IN (SELECT id FROM providers WHERE user_id = $1)))",
    ];
    ...
    const [dataResult, countResult] = await Promise.all([
      ...
      this.pool.query(
        `SELECT COUNT(*)::int AS total FROM disputes ${where}`,
        values,
      ),
    ]);
  ...
  async findDisputes(
    ...
    const resolvedQuery = {
      ...
      jobId: queryDto.jobId
        ? await resolveId(this.pool, "jobs", queryDto.jobId)
        : undefined,
      openedBy: queryDto.openedBy
        ? await resolveId(this.pool, "users", queryDto.openedBy)
        : undefined,
    };
    const { whereClause, values, nextIndex } =
      this.buildWhereClause(resolvedQuery);
