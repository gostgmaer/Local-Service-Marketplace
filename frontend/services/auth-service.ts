...
class AuthService {
  ...
  async checkIdentifier(
    ...
    const response = await apiClient.post<CheckIdentifierResult>(
      "/user/auth/check-identifier",
      { identifier, type },
    );
    return response.data;
  }
  ...
  async verifyEmail(token: string): Promise<void> {
    const response = await apiClient.get<void>(
      `/user/auth/email/verify?token=${encodeURIComponent(token)}`,
    );
    return response.data;
  }

  async resendVerificationEmail(email: string): Promise<void> {
    await apiClient.post<void>("/user/auth/email/resend-verification", {
      email,
    });
  }
  ...
  async refreshToken(refreshToken: string): Promise<AuthResponse> {
    const response = await apiClient.post<AuthResponse>("/user/auth/refresh", {
      refreshToken,
    });
    return response.data;
  }
  ...
  async changePassword(
    ...
    await apiClient.post<void>("/user/auth/change-password", {
      currentPassword,
      newPassword,
    });
  }
  ...
  async setup2FA(): Promise<TwoFASetup> {
    const response = await apiClient.post<TwoFASetup>(
