...
class JobService {
	...
	async startJob(id: string): Promise<Job> {
		const response = await apiClient.patch<Job>(`/jobs/${id}/status`, { status: "in_progress" });
		return response.data;
	}

	async completeJob(id: string): Promise<Job> {
		const response = await apiClient.patch<Job>(`/jobs/${id}/status`, { status: "completed" });
		return response.data;
	}

	async cancelJob(id: string, reason?: string): Promise<Job> {
		const response = await apiClient.patch<Job>(`/jobs/${id}/status`, { status: "cancelled", notes: reason });
		return response.data;
	}
