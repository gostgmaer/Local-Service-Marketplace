      ...
      if (
        ...
        if (standard.success) {
          ...
          return Promise.reject({
            response: { status: standard.statusCode, data: standard },
          });
        }
      }
      return response;
...
class ServerAuthService {
  async loginWithEmail(
    ...
    try {
      const response = await serverClient.post<BackendAuthResponse>(
  ...
  async verifyPhoneOtp(
    ...
    try {
      const response = await serverClient.post<BackendAuthResponse>(
        "/api/v1/user/auth/phone/otp/verify",
        {
          phone,
          code,
        },
  ...
  async verifyEmailOtp(
    ...
    try {
      const response = await serverClient.post<BackendAuthResponse>(
        "/api/v1/user/auth/email/otp/verify",
        {
          email,
          code,
        },
  ...
  async refreshToken(
    ...
    try {
      const response = await serverClient.post<BackendRefreshResponse>(
  ...
  async getProfileFromToken(
    accessToken: string,
  ): Promise<BackendAuthResponse["user"] | null> {
    try {
      const response = await serverClient.get<BackendAuthResponse["user"]>(
