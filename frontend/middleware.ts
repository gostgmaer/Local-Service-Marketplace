...
const PERMISSION_ROUTES: Array<{ prefix: string; permissions: string[] }> = [
  ...
  {
    prefix: "/dashboard/availability",
    permissions: ["provider_availability.manage"],
  },
  { prefix: "/dashboard/browse-requests", permissions: ["requests.browse"] },
  {
    prefix: "/dashboard/my-proposals",
    permissions: ["proposals.create", "proposals.update"],
  },
  {
    prefix: "/dashboard/settings/subscription",
  ...
  // Customer / shared
  {
    ...
    permissions: ["requests.create", "requests.read", "requests.browse"],
  },
...
function matchPermissionRoute(pathname: string) {
  const sorted = [...PERMISSION_ROUTES].sort(
...
export async function middleware(req: NextRequest) {
  ...
  if (isProtected && isLoggedIn) {
    ...
    if (userPermissions.length > 0) {
      ...
      if (matched) {
        const hasAccess = matched.permissions.some((p) =>
      ...
      if (
        matched &&
        (!userRole || !(matched.roles as string[]).includes(userRole))
      ) {
        const forbiddenUrl = new URL("/403", nextUrl);
        forbiddenUrl.searchParams.set("callbackUrl", pathname);
        return NextResponse.redirect(forbiddenUrl);
  ...
  if (isLoggedIn && userRole === "provider" && pathname !== "/onboarding") {
    ...
    const isAllowedPath = PROVIDER_ALLOWED_UNVERIFIED_PREFIXES.some(
      (p) => pathname === p || pathname.startsWith(p + "/"),
    );

    // Block all dashboard access for providers who haven't verified contact
    if (
      !isAllowedPath &&
      !contactVerified &&
