...
describe("Alert Component", () => {
  ...
  it("renders title when provided", () => {
    render(<Alert title="Warning" message="Be careful" />);
  ...
  it("renders success variant", () => {
    const { container } = render(<Alert type="success" message="Done!" />);
    expect(container.querySelector(".bg-green-50")).toBeInTheDocument();
  });

  it("renders warning variant", () => {
    const { container } = render(<Alert type="warning" message="Watch out" />);
  ...
  it("renders icon for each type", () => {
    const { container: info } = render(<Alert type="info" message="msg" />);
    ...
    const { container: error } = render(<Alert type="error" message="msg" />);
    expect(error.querySelector("svg")).toBeInTheDocument();
  });

  it("applies custom className", () => {
    const { container } = render(
      <Alert message="Styled" className="my-alert" />,
    );
