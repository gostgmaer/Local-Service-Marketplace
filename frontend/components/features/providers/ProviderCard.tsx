  ...
  return (
		<Link href={ROUTES.PROVIDER_DETAIL(provider.display_id || provider.id)}>
			<Card className='hover:shadow-lg transition-shadow cursor-pointer h-full'>
				<CardHeader>
					<div className='flex items-start gap-4'>
						...
						<div className='flex-1 min-w-0'>
							<h3 className='text-lg font-semibold text-gray-900 truncate'>{provider.business_name}</h3>
							{rating !== undefined && (
				...
				<CardContent>
					{provider.description && <p className='text-gray-600 text-sm mb-4 line-clamp-2'>{provider.description}</p>}
