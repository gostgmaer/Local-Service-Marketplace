...
export function DataTable<T>({
  ...
  const [activePaginationMode, setActivePaginationMode] = useState<
  ...
  const activeColumnFilters =
    isServerMode && serverColumnFilters ? serverColumnFilters : columnFilters;
  const activePagination = {
    pageIndex:
      isServerMode && typeof serverPageIndex === "number"
        ? serverPageIndex
        : pagination.pageIndex,
    pageSize:
      isServerMode && typeof serverPageSize === "number"
        ? serverPageSize
        : pagination.pageSize,
  };
  const activeSearchTerm =
    isServerMode && typeof serverSearchTerm === "string"
      ? serverSearchTerm
      : searchTerm;
  ...
  const handleColumnFiltersChange: OnChangeFn<ColumnFiltersState> = (
    ...
    const next =
      typeof updater === "function" ? updater(activeColumnFilters) : updater;
    if (!isServerMode) {
  ...
  const table = useReactTable({
    ...
    pageCount: Math.max(
      1,
      Math.ceil(
        (isServerMode ? (serverTotalRows ?? data.length) : data.length) /
          Math.max(1, activePagination.pageSize),
      ),
    ),
  ...
  const onQuickSortChange = (field: string) => {
    if (!field) {
      const resetSort = initialSortField
        ? [{ id: initialSortField, desc: initialSortDirection === "desc" }]
        : [];
      if (!isServerMode) {
    ...
    const nextSort = [
      { id: field, desc: (selected?.direction || "asc") === "desc" },
    ];
    if (!isServerMode) {
      setSorting(nextSort);
      table.setPageIndex(0);
  ...
  const totalRecords = isServerMode
    ? (serverTotalRows ?? data.length)
    : table.getFilteredRowModel().rows.length;
  const totalPages = Math.max(
    1,
    Math.ceil(totalRecords / Math.max(1, activePagination.pageSize)),
  );
  ...
  const toolbarFilterFields = renderToolbarFields
    ? renderToolbarFields(table)
    : toolbarFields;
  const activeQuickSortField = activeSorting[0]?.id || "";
  const defaultSortState: SortingState = initialSortField
    ? [{ id: initialSortField, desc: initialSortDirection === "desc" }]
    : [];
  ...
  const hideableColumns = table
    .getAllLeafColumns()
    .filter((column) => column.id !== "action" && column.getCanHide());
  ...
  const convertRowsToCsv = (rows: T[]) => {
    ...
    const headers = exportableColumns.map(
      (column) => `"${String(column.header).replace(/"/g, '""')}"`,
    );
  ...
  const downloadCsv = () => {
    ...
    anchor.href = url;
    anchor.download = exportFileName.endsWith(".csv")
      ? exportFileName
      : `${exportFileName}.csv`;
    anchor.click();
      ...
      <div className="rounded-xl border border-gray-200/80 bg-white p-3 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="ml-auto flex w-full flex-wrap items-center justify-end gap-2 lg:w-auto">
            ...
            {enableSearch && (
              <div className="relative min-w-[160px] sm:min-w-[260px] flex-1 sm:flex-none">
          ...
          {activePaginationMode === "pagination" ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-2">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                Showing {numberFormatter.format(endRow)} of{" "}
                {numberFormatter.format(filteredCount)} records
              </div>
            </div>
          )}
