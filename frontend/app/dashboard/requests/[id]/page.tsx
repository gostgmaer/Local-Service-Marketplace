			...
			<div className='container-custom py-8'>
				...
				<div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
					...
					<div className='lg:col-span-2'>
						<Card>
							<CardHeader>
								<div className='flex items-start justify-between'>
									<div>
										<h1 className='text-2xl font-bold text-gray-900'>Service Request #{request.id.slice(0, 8)}</h1>
								...
								<div className='space-y-6'>
									...
									<div className='grid grid-cols-2 gap-4 pt-4 border-t'>
										<div>
											...
											<p className='text-lg font-semibold text-gray-900'>{formatCurrency(request.budget)}</p>
										</div>
										<div>
											<p className='text-sm text-gray-500'>Category</p>
											<p className='text-lg font-semibold text-gray-900'>{request.category?.name || "N/A"}</p>
										</div>
									</div>
						...
						<Card className='mt-6'>
							<CardHeader>
								<h2 className='text-xl font-semibold text-gray-900'>Proposals ({proposals?.length || 0})</h2>
							</CardHeader>
							<CardContent>
										...
										{proposals.map((proposal) => (
											<div
												...
												<div className='flex items-start justify-between mb-3'>
													<div>
														<h4 className='font-semibold text-gray-900'>{proposal.provider?.name || "Provider"}</h4>
														<p className='text-sm text-gray-500'>{formatRelativeTime(proposal.created_at)}</p>
													</div>
												...
												<div className='flex items-center gap-4 mb-3 text-sm'>
													<span className='font-medium text-gray-900'>Price: {formatCurrency(proposal.price)}</span>
													{proposal.estimated_hours && (
														<>
															<span>•</span>
															<span className='text-gray-600'>Duration: {proposal.estimated_hours} hours</span>
														</>
													)}
												</div>
								...
								:	<p className='text-center py-8 text-gray-500'>No proposals yet</p>}
							</CardContent>
						</Card>
					</div>
