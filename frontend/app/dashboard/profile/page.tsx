							...
							<div className='flex flex-col sm:flex-row sm:items-end sm:justify-between -mt-16'>
								<div className='flex flex-col sm:flex-row sm:items-end'>
									...
									<div className='mt-4 sm:mt-0 sm:ml-6'>
										...
										<p className='text-lg text-gray-600 dark:text-gray-400 capitalize'>{profile?.role || user?.role}</p>
										{isProvider && providerProfileRating !== undefined ?
											<div className='flex items-center mt-2'>
												<Star className='w-5 h-5 text-yellow-400 fill-current' />
												<span className='ml-1 text-lg font-semibold text-gray-900 dark:text-white'>
													{providerProfileRating.toFixed(1)}
												</span>
												<span className='ml-2 text-gray-600 dark:text-gray-400'>({reviews?.length ?? 0} reviews)</span>
											</div>
										:	null}
						...
						<div className='md:col-span-2 space-y-6'>
							...
							{isProvider && providerProfile?.description && (
								<div className='bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6'>
									...
									<p className='text-gray-600 dark:text-gray-400'>{providerProfile.description}</p>
									...
									<div className='space-y-4'>
										{reviews.slice(0, 3).map((review: any) => (
											<div
												...
												{review.comment && <p className='text-gray-700 dark:text-gray-300 text-sm'>{review.comment}</p>}
											</div>
										))}
										{reviews.length > 3 && (
											<Link
