...
const mapUserSortBy = (
  field?: string,
): "createdAt" | "email" | "name" | "role" | "lastLoginAt" => {
  switch (field) {
    case "email":
      return "email";
    case "name":
...
export default function AdminUsersPage() {
  ...
  const roleFilter = String(
    serverFilters.find((f) => f.id === "role")?.value || "",
  );
            ...
            <CardContent>
              {isLoading && !users ? (
                ...
                <DataTable<UserRow>
                        ...
                        >
                          ...
                          {USER_ROLE_OPTIONS.map((role) => (
                            <option key={role} value={role}>
                              {ROLE_LABELS[role]} (
                              {userStats?.byRole[
                                role as keyof typeof userStats.byRole
                              ] ?? 0}
                              )
                            </option>
                          ))}
                        ...
                        >
                          ...
                          {USER_STATUS_OPTIONS.map((status) => (
                            <option key={status} value={status}>
                              ...
                              {userStats?.byStatus[
                                status as keyof typeof userStats.byStatus
                              ] ?? 0}
                              )
                            </option>
                          ))}
                  ...
                  onEmptyAction={() =>
                    router.push(ROUTES.DASHBOARD_ADMIN_USERS_CREATE)
                  }
