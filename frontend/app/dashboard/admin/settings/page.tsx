...
function resolveType(row: SettingRow): SettingType {
  ...
  if (t === "boolean" || t === "number" || t === "textarea" || t === "text")
    return t;
...
const GROUP_CONFIG: Record<
  string,
  { label: string; icon: any; keys: string[] }
> = {
  platform: {
    label: "Platform & Fees",
    icon: IndianRupee,
...
function SettingItem({
    ...
    return (
      <div className="flex items-start justify-between py-3 border-b border-gray-100 dark:border-gray-800 last:border-0">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isOn ? "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}
          >
            {isOn ? "ON" : "OFF"}
          </span>
          <Toggle
  ...
  return (
    <div
      className={`py-3 border-b border-gray-100 dark:border-gray-800 last:border-0 ${isTextarea ? "block" : "flex items-start justify-between"}`}
    >
      <div className={isTextarea ? "w-full" : "flex-1 mr-4 min-w-0"}>
        <div
          className={`flex items-start ${isTextarea ? "justify-between mb-1" : ""}`}
        >
          <div>
            <p className="font-medium text-gray-900 dark:text-white text-sm font-mono">
              {setting.key}
        ...
        {isTextarea && !editing && (
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400 font-mono whitespace-pre-wrap break-all bg-gray-50 dark:bg-gray-800/50 rounded px-2 py-1">
            {setting.value}
          </p>
        )}
        {editing && (
      ...
      {!isTextarea && !editing && (
        <div className="flex items-center gap-3 flex-shrink-0">
          <span
            className="text-sm text-gray-600 dark:text-gray-400 font-mono max-w-[180px] truncate"
            title={setting.value}
          >
            {setting.value}
          </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              value={form.type}
              onChange={(e) =>
                setForm((f) => ({ ...f, type: e.target.value as SettingType }))
              }
          ...
          <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm">
                Changes take effect immediately — boolean settings update on
                toggle, others on Save.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <Button
          ...
          {isLoading ? (
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm">
                  Run the database migrations to seed the default system
                  settings.
                </p>
              </CardContent>
            </Card>
