...
export default function EditRolePage() {
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      rbacService.assignPermissions(roleId, {
  ...
  const permissionGrid = useMemo(() => {
    if (!allPermissions)
      return { resources: [], actions: new Set<string>(), map: new Map() };
        ...
        <div className="container-custom py-12">
          ...
          {roleError ? (
            <ErrorState
              ...
              retry={() => router.push(ROUTES.DASHBOARD_ADMIN_ROLES)}
            />
          ) : isLoading ? (
            <Card>
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Click cells to toggle. Click resource names to toggle entire
                  rows.
                </p>
              </CardHeader>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-800 sticky top-0">
                      <tr>
                        <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 min-w-[160px]">
                          Resource
                        </th>
                        {sortedActions.map((action) => (
                          <th
                            key={action}
                            className="text-center px-2 py-3 font-medium text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide min-w-[70px]"
                          >
                            {action.replace("_", " ")}
                          </th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                      {permissionGrid.resources.map((resource) => {
                        const permsForRow = sortedActions.map((action) =>
                          permissionGrid.map.get(`${resource}:${action}`),
                        );
                        const assignedCount = permsForRow.filter(
                          (p) => p && selectedIds.has(p.id),
                        ).length;
                        ...
                        return (
                          ...
                          >
                            ...
                            {sortedActions.map((action) => {
                              const perm = permissionGrid.map.get(
                                `${resource}:${action}`,
                              );
                              if (!perm) {
                                return (
