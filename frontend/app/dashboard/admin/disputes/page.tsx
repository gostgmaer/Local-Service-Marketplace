...
const DISPUTE_STATUS_LABELS: Record<
  (typeof DISPUTE_STATUS_OPTIONS)[number],
  string
> = {
  open: "Open",
...
const mapDisputeSortBy = (
  field?: string,
): "createdAt" | "status" | "resolvedAt" => {
  switch (field) {
    case "status":
      return "status";
    default:
                        ...
                        {DISPUTE_STATUS_OPTIONS.map((status) => (
                          <option key={status} value={status}>
                            ...
                            {disputeStats?.byStatus[
                              status as keyof typeof disputeStats.byStatus
                            ] ?? 0}
                            )
                          </option>
                        ))}
