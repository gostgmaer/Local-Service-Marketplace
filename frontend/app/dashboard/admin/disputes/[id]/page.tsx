...
const RESOLUTION_OUTCOMES = [
  {
    value: "award_customer",
    label: "Award Customer (issue refund)",
    requiresRefund: true,
  },
  {
    value: "award_provider",
    label: "Award Provider (no refund)",
...
export default function AdminDisputeDetailPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const selectedOutcome = RESOLUTION_OUTCOMES.find(
        (o) => o.value === outcome,
      );
      const newStatus =
        outcome === "award_customer" || outcome === "award_provider"
          ? "resolved"
          : outcome;
      ...
      if (selectedOutcome?.requiresRefund && dispute.job_id) {
        try {
          const payments = await paymentService.getPaymentsByJob(
            dispute.job_id,
          );
          ...
          if (completedPayment) {
            ...
            toast("Refund initiated for the associated payment", {
              icon: "💳",
            });
          }
        } catch (refundErr: any) {
          ...
          const msg =
            refundErr?.response?.data?.message ??
            refundErr?.message ??
            "Refund error";
      ...
      return adminService.updateDispute(dispute.id, {
        status: newStatus,
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <select
                              ...
                              value={outcome}
                              onChange={(e) =>
                                setOutcome(e.target.value as OutcomeValue)
                              }
                            >
                          ...
                          {RESOLUTION_OUTCOMES.find((o) => o.value === outcome)
                            ?.requiresRefund && (
                            <div className="flex items-start gap-2 p-3 rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700">
                              ...
                              <p className="text-sm text-amber-700 dark:text-amber-300">
                                A refund will be issued if a completed payment
                                exists for the associated job.
                              </p>
                            </div>
                          )}
                          <div>
