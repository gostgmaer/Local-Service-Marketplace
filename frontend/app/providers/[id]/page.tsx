...
const AvailabilitySchedule = dynamic(
  () =>
    import("@/components/features/providers/AvailabilitySchedule").then(
      (m) => ({ default: m.AvailabilitySchedule }),
    ),
  {
    ssr: false,
    loading: () => (
...
export default function ProviderDetailPage() {
  ...
  const professionalServiceJsonLd = {
    ...
    description:
      ...
      `${provider.business_name} — verified local service provider on Local Service Marketplace.`,
    ...
    hasOfferCatalog: provider.services?.length
      ? {
          ...
          itemListElement: (provider.services || []).map((svc, i) => ({
            ...
            itemOffered: {
              "@type": "Service",
              name: categoryMap[svc.category_id]?.name || "Service",
            },
          })),
        }
      : undefined,
                  ...
                  {reviewAggregates && reviewAggregates.total_reviews > 0 && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-semibold">
                        {reviewAggregates.average_rating.toFixed(1)}
                      </span>
                    </div>
                  )}
                </div>
                    ...
                    {[
                      ...
                      const pct =
                        reviewAggregates.total_reviews > 0
                          ? Math.round(
                              (count / reviewAggregates.total_reviews) * 100,
                            )
                          : 0;
                      return (
                        <div
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    disabled={
                      toggleFavoriteMutation.isPending || checkingFavorite
                    }
                  >
                    <Heart
