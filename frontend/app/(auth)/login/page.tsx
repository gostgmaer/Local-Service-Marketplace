...
const loginSchema = z.object({
  identifier: z
    ...
    .refine(
      ...
      {
        message:
          "Enter a valid email (e.g., user@example.com) or phone (e.g., +1234567890)",
      },
    ),
...
function LoginContent() {
  ...
  const [step, setStep] = useState<"identifier" | "method" | "authenticate">(
    "identifier",
  );
  ...
  const [identifierExists, setIdentifierExists] = useState<boolean | null>(
    null,
  );
  ...
  const checkIdentifierExists = async (
    ...
    try {
      ...
      const payload = await authService.checkIdentifier(
    ...
    <div className="min-h-screen flex items-center justify-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary-50/50 via-white to-white dark:from-gray-900 dark:via-gray-900 dark:to-gray-950 py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          <Link
            href="/"
            className="inline-block text-xl font-bold text-primary-600 dark:text-primary-400 mb-6"
          >
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-soft ring-1 ring-inset ring-black/[0.04] dark:ring-white/[0.04] p-8 space-y-6">
          ...
          <div className="flex items-center justify-center space-x-2 text-sm">
            <div
              className={`flex items-center ${detectedType !== "unknown" ? "text-green-600 dark:text-green-400" : "text-blue-600 dark:text-blue-400"}`}
            >
              <span
                className={`w-6 h-6 rounded-full flex items-center justify-center ${detectedType !== "unknown" ? "bg-green-600 text-white" : "bg-blue-600 text-white"}`}
              >
            ...
            <div className="w-8 h-0.5 bg-gray-300 dark:bg-gray-600"></div>
            <div
              className={`flex items-center ${step === "authenticate" ? "text-blue-600 dark:text-blue-400" : "text-gray-400"}`}
            >
              <span
                className={`w-6 h-6 rounded-full flex items-center justify-center ${step === "authenticate" ? "bg-blue-600 text-white" : "bg-gray-300 dark:bg-gray-600 text-white"}`}
              >
                3
              </span>
          ...
          >
              ...
              <div>
                ...
                {checkingIdentifier && detectedType !== "unknown" && (
                  <p className="mt-1 text-sm text-blue-600 dark:text-blue-400 flex items-center">
                    ...
                    Checking if {detectedType === "email" ? "email" : "phone"}{" "}
                    is registered...
                  </p>
                )}
                  ...
                  detectedType !== "unknown" && (
                    <div className="mt-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                      <p className="text-sm text-amber-800 dark:text-amber-200">
                        ⚠ This{" "}
                        {detectedType === "email" ? "email" : "phone number"} is
                        not registered.
                      </p>
                      <Link
                        href="/signup"
              ...
              {(step === "method" || step === "authenticate") &&
                identifierExists && (
                  <div>
                    ...
                    <div
                      className={`grid ${availableMethods.includes("otp") ? "grid-cols-2" : "grid-cols-1"} gap-3`}
                    >
                    ...
                    <p className="mt-2 text-sm text-green-600 dark:text-green-400 text-center flex items-center justify-center">
                      ...
                      {detectedType === "email" ? "Email" : "Phone number"}{" "}
                      verified. Choose how to continue.
                    </p>
            ...
            {step === "authenticate" &&
              loginMethod === "password" &&
              (detectedType === "email" || detectedType === "unknown") && (
