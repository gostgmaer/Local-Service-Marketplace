...
function UnsubscribeContent() {
  ...
  const handleUnsubscribe = async (emailAddress: string) => {
    try {
      const data = await notificationService.unsubscribe(
        emailAddress,
      ...
      if (data.message?.includes("already")) {
        ...
        setStatus("success");
        setMessage(
          `${emailAddress} has been successfully unsubscribed from our emails.`,
        );
      }
    } catch (error: any) {
      ...
      <div className="max-w-md w-full">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 text-center">
          ...
          {status === "success" && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6 text-left">
              ...
              <ul className="text-sm text-blue-700 dark:text-blue-400 mt-2 space-y-1 list-disc list-inside">
                ...
                <li>
                  You'll still get transactional emails (job updates, payments)
                </li>
          ...
          {status === "already" && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6 text-left">
              <p className="text-sm text-blue-800 dark:text-blue-300">
                This email address was previously unsubscribed from our mailing
                list. You won't receive any promotional emails.
              </p>
            </div>
          )}
        ...
        <p className="mt-6 text-center text-xs text-gray-500 dark:text-gray-400">
          Note: Unsubscribing only affects promotional emails. You'll still
